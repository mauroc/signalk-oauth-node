    ...
    primus.on('connection', function(spark) {
        ...
        if  (spark.query.user_id && spark.query.token) {
            User.findOne({ 'squiddio.id' : spark.query.user_id.toString() },function(err,user) {
                ...
                if (user && (user.squiddio.token ==  spark.query.token) && (currTimeInSecs < user.squiddio.tokenExpires)) {
                    ...
                    spark.write({status: 'invalid or expired credentials'});
                    //spark.end();
                }
            });
        } else {
            spark.write({status: 'missing credentials'});
            //spark.end();
        }
    });
