...
/* squiddio production database version. Uses direct mysql connection go BBX to validate tokens */
...
var config;
try {
    // create /settings/credentials.json like this: {"mysql_pwd": "my_very_secret_mysql_password"}
    // add /settings/credentials.json to .gitignore
    config = require("../../settings/credentials.json");
}
catch (err) {
    config = {};
    ...
    api.start = function() {
        ...
        primus.on('connection', function(spark) {
            ...
            if (spark.query.user_id && spark.query.token) {
                var token, boat_id, user_id;

                mysql_conn.query("SELECT * FROM `oauth_access_tokens` WHERE token = '"+spark.query.token+"' LIMIT 1", function(err, row, fields) {
                    if (!err)
                        token = row[0];
                    else
                ...
                if (app_id) {
                    mysql_conn.query("SELECT * FROM oauth_applications WHERE id = "+app_id+" LIMIT 1", function(err, row, fields) {
                        if (!err)
                ...
                if (boat_id) {
                    mysql_conn.query("SELECT * FROM boats WHERE id = "+boat_id+" AND currently_onboard = 1", function(err, row, fields) {
                        if (!err)
                ...
                if (user_id) {
                    ...
                    if (currTimeInSecs < created_at.getTime()/1000+expires_in){
                    ...
                    } else {spark.write({status: 'expired Oauth token'});}
