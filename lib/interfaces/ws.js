                ...
                if (!mysqlConn){
                    ...
                    mysqlConn.query("SELECT * FROM `oauth_access_tokens` WHERE token = '"+spark.query.token+"' LIMIT 1", function(err, row, fields) {
                        if (row[0] && !err) {
                            ...
                            } else spark.write({"error": "oauth: incorrect token for user " + user_id, requestUrl: reqPath});
                        } else spark.write({"error": "oauth: missing or incorrect token", requestUrl: reqPath });
                    });
                }
