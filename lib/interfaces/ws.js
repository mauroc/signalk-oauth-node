   ...
   api.start = function() {
        ...
        primus.on('connection', function(spark) {
            ...
            if  (spark.query.user_id && spark.query.token) {
                ...
                mysql_req= connection.query("SELECT * from users WHERE id = "+spark.query.user_id, function(err, rows, fields) {
                    if (!err)
                        return rows[0];
                ...
                if (user && (user.squiddio.token ==  spark.query.token)) {
                        ...
                        if (currTimeInSecs < user.squiddio.tokenExpires){
                    ...
                    } else { spark.write({status: 'invalid Oauth token'}); }
                });
