...
fs = require("fs")
var fileName = "./settings/credentials.json"
var config
   ...
   api.start = function() {
        ...
        primus.on('connection', function(spark) {
            ...
            if (spark.query.user_id && spark.query.token) {
                ...
                connection.query("SELECT * FROM `oauth_access_tokens` WHERE token = '"+spark.query.token+"' LIMIT 1", function(err, row, fields) {
                    if (!err)
                        token = row[0];
                });
                ...
                if (app_id) {
                    connection.query("SELECT * FROM oauth_applications WHERE id = "+app_id+" LIMIT 1", function(err, row, fields) {
                        if (!err)
                ...
                if (boat_id) {
                    connection.query("SELECT * FROM boats WHERE id = "+boat_id+" AND currently_onboard = 1", function(err, row, fields) {
                        if (!err)
